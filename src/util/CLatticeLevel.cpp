...
void CLatticeLevel::GenerateFirstLevel(std::vector<std::unique_ptr<util::CLatticeLevel>>& levels, const PatternColumnLayoutRelationData* relation) {
        ...
        auto const* empty_vertex = new util::CLatticeVertex(
        ...
        for (auto const& rhs_column_patterns : relation->GetPatternColumnData()) {
            for (const auto& [kRhsPatternValue, kPatternColumnData] : rhs_column_patterns) {
                rhs_candidates.emplace_back(new ColumnPattern(kPatternColumnData.GetColumn(), kRhsPatternValue));
            }
        }

        for (size_t col_index = 0; col_index != col_numbers; ++col_index) {
            const auto& column_patterns = relation->GetColumnPatterns(col_index);
            ...
            for (const auto& [pattern_value, column_pattern_data] : column_patterns) {
                const auto* pli = column_pattern_data.GetPatternPositionListIndex();
                std::vector<int> pattern_values(col_numbers);
                pattern_values[col_index] = pattern_value;
                auto vertex =
                        std::make_unique<util::CLatticeVertex>(
                                relation,
                                TuplePattern(relation, std::move(pattern_values), indices)
                        );
...
void CLatticeLevel::GenerateNextLevel(std::vector<std::unique_ptr<CLatticeLevel>>& levels) {
    ...
    for (unsigned int vertex_index_1 = 0; vertex_index_1 < current_level_vertices.size();
        ...
        for (unsigned int vertex_index_2 = vertex_index_1 + 1;
            ...
            for (unsigned int i = 0, skip_index = parent_indices.find_first(); i < current_level_index;
