...
public:
    ...
    void Execute() {
        ...
        while (chunk_generator_.HasNext()) {
            ...
            ProcessChunk(start, end, chunk_generator_.ChunksNumber() > 1);
        };
...
private:
    ...
    void ProcessChunk(BufferPtr start, BufferPtr end, bool is_chunk) override {
        ...
        spilled_manager_.SpillColumnsToDisk(adapter.to_print, GetSortedColumns(adapter), is_chunk);
    }
    ...
    void MemoryLimitProcess(Adapter const& adapter, std::size_t current_row) {
        if (IsLimitReached(current_row)) {
            spilled_manager_.SpillColumnsToDisk(adapter.to_print, GetSortedColumns(adapter), true);
        }
    }
...
private:
    ...
    void ReserveColumns(const Adapter& adapter) final {
        this->columns_.assign(this->GetHeaderSize(), Column{adapter.less});
    }
    ...
    void Sort(Adapter const& adapter) {
        ...
        for (std::size_t j = 0; j < this->GetHeaderSize(); ++j) {
            threads.emplace_back([&col = this->columns_[j], &adapter]() {
                std::sort(col.begin(), col.end(), adapter.less);
                col.erase(unique(col.begin(), col.end(), adapter.eq), col.end());
            });
...
public:
    VectorBasedTableProcessor(SpilledFilesManager& spilled_manager,
                         const BaseTableProcessor::DatasetConfig& config, std::size_t memory_limit,
...
using ChunkProcessor = std::conditional_t<ColTypeImpl::SET == col,
                                          SetBasedTableProcessor<key>,
