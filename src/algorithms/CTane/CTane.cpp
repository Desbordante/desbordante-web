...
bool CTane::IsExactCfd(const CLatticeVertex & x_vertex, const CLatticeVertex & xa_vertex) const {
...
double CTane::CalculateConstConfidence(const CLatticeVertex & x_vertex, const CLatticeVertex & xa_vertex) const {
    return (double)xa_vertex.GetSupport()/ x_vertex.GetSupport();
}

double CTane::CalculateConfidence(const CLatticeVertex & x_vertex, const CLatticeVertex & xa_vertex) const {
    auto error_sum =
        CalculatePartitionError(*x_vertex.GetPositionListIndex(), *xa_vertex.GetPositionListIndex());
    return 1 - (double)error_sum / x_vertex.GetSupport();
}

double CTane::CalculatePartitionError(const PatternPositionListIndex & x_pli, const PatternPositionListIndex & xa_pli) const {
    int error_sum = 0;
...
void CTane::RegisterCfd(const TuplePattern& lhs_pattern, const ColumnPattern& rhs_pattern, unsigned supp, double conf) {
    CFD cfd(lhs_pattern, rhs_pattern);
    LOG(DEBUG) << "Discovered CFD: " << cfd.ToString(ItemNames())
...
void CTane::PruneCandidates(CLatticeLevel *level, const CLatticeVertex * x_vertex,
                            const CLatticeVertex * xa_vertex, ColumnPattern const& rhs_column_pattern) const {
    // otherIndices = { idx : idx in R\X }
    auto other_indices =
            ~boost::dynamic_bitset<>(relation_->GetSchema()->GetNumColumns())
            -
    ...
    for (const auto &vertex : level->GetVertices()) {
        ...
        if (tuple_pattern.HasColumnPattern(rhs_column_pattern)
            &&
            xa_vertex_without_col <= tuple_pattern.GetWithoutColumn(rhs_col_index)) {
            candidates.erase(
                    std::remove_if(
    ...
    level_vertices.erase(
            std::remove_if(level_vertices.begin(), level_vertices.end(),
                    [&](std::unique_ptr<CLatticeVertex> & vertex) {
                        return vertex->GetRhsCandidates().empty() || (vertex->GetPositionListIndex()->GetSize() < min_sup_);
                    }),
        level_vertices.end());
}

unsigned long long CTane::ExecuteInternal() {
    double progress_step = 100.0 / std::min(max_arity_, (unsigned int)relation_->GetSchema()->GetNumColumns());
    auto start_time = std::chrono::system_clock::now();

    std::vector<std::unique_ptr<CLatticeLevel>> levels;

    for (unsigned int arity = 0; arity < max_arity_; arity++) {
        if (arity == 0) {
        ...
        LOG(DEBUG) << "Checking " << level->GetVertices().size() << " "
        ...
        for (auto& xa: level->GetVertices()) {
            ...
            for (const auto x: xa->GetParents()) {
                ...
                for (const auto* rhs_column_pattern : kRhsCandidates) {
                    ...
                    if ((x->GetTuplePattern().IsConst() && rhs_column_pattern->IsVar())
                        || (x->GetTuplePattern().IsVar() && rhs_column_pattern->IsConst())) {
                        continue;
                    }

                    double conf = min_conf_ == 1
                            ? IsExactCfd(*x, *xa)
                    ...
                    if (conf >= min_conf_) {
                        RegisterCfd(x->GetTuplePattern(), *rhs_column_pattern, xa->GetSupport(), conf);
                    }
                    if (conf == 1) {
    ...
    LOG(DEBUG) << "Found " << cfd_collection_.size() << " CFDs\n";
