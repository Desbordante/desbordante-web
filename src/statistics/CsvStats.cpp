...
Statistic CsvStats::GetMin(unsigned index) {
    if(allStats[index].min.hasValue())
        return allStats[index].min.copy(config_.is_null_equal_null);
...
Statistic CsvStats::GetMax(unsigned index) {
    if(allStats[index].max.hasValue())
        return allStats[index].max.copy(config_.is_null_equal_null);
    mo::TypedColumnData const& col = col_data_[index];
    mo::TypeId type_id = col.GetTypeId();
    if (type_id == +mo::TypeId::kEmpty || type_id == +mo::TypeId::kNull ||
        type_id == +mo::TypeId::kUndefined) {
...
Statistic CsvStats::GetSum(unsigned index) {
    if(allStats[index].sum.hasValue())
        return allStats[index].sum.copy(config_.is_null_equal_null);
    mo::TypedColumnData const& col = col_data_[index];
    mo::TypeId type_id = col.GetTypeId();
    if (type_id == +mo::TypeId::kEmpty || type_id == +mo::TypeId::kNull ||
        type_id == +mo::TypeId::kUndefined) {
...
Statistic CsvStats::GetAvg(unsigned index) {
    if(allStats[index].avg.hasValue())
        return allStats[index].avg.copy(config_.is_null_equal_null);
    mo::TypedColumnData const& col = col_data_[index];
    mo::TypeId type_id = col.GetTypeId();
    if (type_id == +mo::TypeId::kEmpty || type_id == +mo::TypeId::kNull ||
        type_id == +mo::TypeId::kUndefined) {
...
Statistic CsvStats::GetSTD(unsigned index) {
    if(allStats[index].STD.hasValue())
        return allStats[index].STD.copy(config_.is_null_equal_null);
...
Statistic CsvStats::GetSkewness(unsigned index) {
    if(allStats[index].skewness.hasValue())
        return allStats[index].skewness.copy(config_.is_null_equal_null);
    mo::TypedColumnData const& col = col_data_[index];
    mo::TypeId type_id = col.GetTypeId();
    if (type_id == +mo::TypeId::kEmpty || type_id == +mo::TypeId::kNull ||
        type_id == +mo::TypeId::kUndefined) {
...
Statistic CsvStats::GetKurtosis(unsigned index) {
    if(allStats[index].kurtosis.hasValue())
        return allStats[index].kurtosis.copy(config_.is_null_equal_null);
    mo::TypedColumnData const& col = col_data_[index];
    mo::TypeId type_id = col.GetTypeId();
    if (type_id == +mo::TypeId::kEmpty || type_id == +mo::TypeId::kNull ||
        type_id == +mo::TypeId::kUndefined) {
...
int CsvStats::Distinct(unsigned index) {
    if(allStats[index].isDistinctCorrect)
        return allStats[index].distinct;
    mo::TypedColumnData const& col = col_data_[index];
    mo::TypeId type_id = col.GetTypeId();
    if (type_id == +mo::TypeId::kEmpty || type_id == +mo::TypeId::kNull ||
        type_id == +mo::TypeId::kUndefined) {
    ...
    allStats[index].distinct = set.size();
    allStats[index].isDistinctCorrect = true;
    return set.size();
}
...
Statistic CsvStats::GetQuantile(double part, unsigned index) {
    ...
    if(!allStats[index].quantile25.hasValue()) {
        std::sort(data.begin(), data.end(), type.GetComparator());
        allStats[index].quantile25 = Statistic(type.Clone(data[(int)(data.size() * 0.25)]),
                     Statistic::getTypeClone(col.GetType(), config_.is_null_equal_null));
        allStats[index].quantile50 = Statistic(type.Clone(data[(int)(data.size() * 0.50)]),
                     Statistic::getTypeClone(col.GetType(), config_.is_null_equal_null));
        allStats[index].quantile75 = Statistic(type.Clone(data[(int)(data.size() * 0.75)]),
                     Statistic::getTypeClone(col.GetType(), config_.is_null_equal_null));
    } else
...
unsigned long long CsvStats::Execute() {
    for (unsigned i = 0; i < allStats.size(); ++i) {
        ...
        Distinct(i);
        // обязательно после distinct, чтобы не проседала производительность
