...
Statistic Statistic::copy(bool is_null_eq_null) const {
    return Statistic(type->Clone(data), getTypeClone(*type, is_null_eq_null));
}
...
static void inline addStatToSS(std::stringstream& ss, const Statistic& stat, const std::string& statName) {
    if(stat.hasValue())
        ss << " \"" << statName << "\" = '" << stat.toString() << "',";
}

std::string ColumnStats::toDBUpdate() const {
    std::stringstream res;
    res << "\"count\" = '" << count << "',";
    res << " \"distinct\" = '" << distinct << "',";
    addStatToSS(res, avg, "avg");
    addStatToSS(res, STD, "STD");
    addStatToSS(res, skewness, "skewness");
    addStatToSS(res, kurtosis, "kurtosis");
    addStatToSS(res, min, "min");
    addStatToSS(res, max, "max");
    addStatToSS(res, sum, "sum");
    addStatToSS(res, quantile25, "quantile25");
    addStatToSS(res, quantile50, "quantile50");
    addStatToSS(res, quantile75, "quantile75");
    res << " \"isCategorical\" = '" << (isCategorical ? "true" : "false") << "'";
    return res.str();
}
