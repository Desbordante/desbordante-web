...
const FilePropsList: FC<Props & FormProps> = ({data, onDelete, switchEdit}) => {
    const separators = {",": 'Comma ","', ";": 'semicolon ";"', "|": 'Pipe "|"'}
    return <>
        <div className={styles.propsList}>
            <div><p>File type <Tooltip>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua</Tooltip></p><p>{data.fileType || 'CSV'}</p></div>
            <div><p>Separator</p><p>{_.get(separators, data.delimiter, data.delimiter)}</p></div>
            <div><p>Has header row</p><p>{data.hasHeader ? 'Yes' : 'No'}</p></div>
            <div><p>Itemset format <Tooltip>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua</Tooltip></p><p>{data.inputFormat}</p></div>
            <div><p>ID column index</p><p>{data.tidColumnIndex}</p></div>
            <div><p>Itemset column index</p><p>{data.itemColumnIndex}</p></div>
        </div>
        <div className={styles.buttonsRow}>
            <Button variant="secondary-error" onClick={onDelete}>Delete</Button>    
            <Button variant="primary" onClick={switchEdit}>Edit</Button> 
        </div>
    </>
}
...
const FilePropsForm: FC<Props & FormProps> = ({data, switchEdit}) => {
    ...
    const onSubmit: SubmitHandler<FieldValues> = data => switchEdit && switchEdit()
    ...
    return <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className={styles.buttonsRow}> 
                <Button variant="secondary" onClick={switchEdit}>Cancel</Button>    
...
const FilePropsView: FC<Props> = ({data, onClose}) => {
    ...
    return (
        <div className={styles.container}>        
            ...
            <TabView>
                <Tab name="Properties">
                    {!isEdited && <FilePropsList onDelete={onClose} switchEdit={switchEdit} data={data} />}
